% run_pom_paci2020_nodal.m
% Population-of-Models (PoM) simulation using Paci2020 model for Nodal Phenotype
...
phenotype = 'nodal';  % Changed to nodal phenotype
t_fixed = 0:0.001:800;

%% Initial conditions (load nodal baseline state)
load('Y0_nodal.mat');  % You must generate and save this beforehand
Y0 = Y0_nodal;
...
%% Output directory creation (specific for nodal models)
output_dir = 'results/nodal_models/';
...
parfor model_idx = 1:numModels
    ...
    fprintf('Running nodal model %d/%d...\n', model_idx, numModels);
...
fprintf('Nodal PoM generation completed successfully.\n');
...
%% Visualization from individual files (nodal-specific)
...
xlabel('Time (s)');
ylabel('Vm (mV)');
title(sprintf('Population of %d Nodal Models (LHS ±30%%)', numModels));
grid on;
xlim([790 800]);
results_dir = 'results/';
if ~exist(results_dir, 'dir'), mkdir(results_dir); end
saveas(gcf, fullfile(results_dir, 'Population_of_Nodal_Models.png'));
